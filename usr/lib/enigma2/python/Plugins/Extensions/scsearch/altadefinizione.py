                ...
                if 'gzip' in content_encoding:
                    try:
                        ...
                        with gzip.GzipFile(fileobj=io.BytesIO(content), mode='rb') as f:
                            ...
                            log.info(
                                "Altadefinizione: Decompressed gzip, {} chars".format(
                                    len(html)))
                    except Exception as e:
                        log.warning(
                            "Altadefinizione: Gzip decompression failed: {}, using raw content".format(e))
                        html = response.text
                else:
                    # Not compressed - use directly
                    html = response.text
                    log.info(
                        "Altadefinizione: Response not compressed, {} chars".format(
                            len(html)))
        ...
        try:
            ...
            try:
                with open('/tmp/altadef_html_decompressed.txt', 'w', encoding='utf-8') as f:
                    f.write(html[:15000])
                log.info(
                    "Altadefinizione: HTML saved to /tmp/altadef_html_decompressed.txt")
    ...
    def _parse_search_results(self, html, media_type='movie'):
        ...
        try:
            with open('/tmp/altadef_html_debug.txt', 'w', encoding='utf-8') as f:
                f.write(html[:15000])
            log.info(
                "Altadefinizione: HTML saved to /tmp/altadef_html_debug.txt (first 15000 chars)")
        except Exception as e:
        ...
        if matches:
            for i, (url, title) in enumerate(matches[:3]):
                log.info(
                    "Altadefinizione: Match {} - URL: {}, Title: {}".format(i + 1, url, title.strip()))
        else:
            ...
            log.info(
                "Altadefinizione: Fallback found {} links".format(
                    len(fallback_matches)))
            if fallback_matches:
                for i, url in enumerate(fallback_matches[:5]):
                    log.info(
                        "Altadefinizione: Fallback link {}: {}".format(
                            i + 1, url))
        ...
        for url, title in matches:
            ...
            if card_match:
                rating_match = re.search(
                    r'<span class="label rate">([\d.]+)</span>',
                    card_match.group(0))
                if rating_match:
    ...
    def get_streaming_links(self, movie_url):
        ...
        try:
            ...
            if iframe_match:
                ...
                log.info(
                    "Altadefinizione: Found embed URL: {}".format(embed_url))
        ...
        try:
            ...
            if 'gzip' in content_encoding:
                try:
                    ...
                    log.warning(
                        "Gzip decompression failed: {}, using raw content".format(e))
                    html = response.text
                ...
                try:
                    ...
                    log.info(
                        "Altadefinizione: Decompressed brotli, {} chars".format(
                            len(html)))
                except BaseException:
            ...
            try:
                with open('/tmp/vixsrc_decompressed.html', 'w', encoding='utf-8') as f:
                    f.write(html)
                log.info(
                    "Altadefinizione: Decompressed HTML saved to /tmp/vixsrc_decompressed.html")
            except BaseException:
                pass
            ...
            if matches:
                ...
                log.info(
                    "Altadefinizione: Found master playlist: {}".format(stream_url))
                return stream_url
            ...
            for pattern in json_patterns:
                ...
                if matches:
                    ...
                    log.info(
                        "Altadefinizione: Found master playlist in JSON: {}".format(stream_url))
                    return stream_url
            ...
            for pattern in js_patterns:
                ...
                if match:
                    ...
                    if not stream_url.startswith('http'):
                        stream_url = 'https://vixsrc.to' + stream_url
                    log.info(
                        "Altadefinizione: Found master playlist in JS: {}".format(stream_url))
                    return stream_url
            ...
            if iframe_match:
                ...
                if not new_url.startswith('http'):
                    new_url = 'https:' + \
                        new_url if new_url.startswith('//') else 'https://' + new_url
                log.info(
                    "Altadefinizione: Found nested iframe: {}".format(new_url))
                return self._extract_vixsrc_stream(new_url, embed_url)

            log.warning("Altadefinizione: No stream URL found.")
            log.warning(
                "Altadefinizione: First 1000 chars of decompressed HTML:")
